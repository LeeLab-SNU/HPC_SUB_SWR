...
if exist(['behaviorEpoch_rat' thisRID '.xlsx'])
    ...
    thisEPOCH = readmatrix(['behaviorEpoch_rat' thisRID '.csv']);
end
...
elseif exist(['TT' thisTTID '_beh_SS_' jmnum2str(str2double(thisCLID),2) '.ntt'],'file')
    thisEpochCLTS = Nlx2MatSpike(['TT' thisTTID '_beh_SS_' jmnum2str(str2double(thisCLID),2) '.ntt'], [1 0 0 0 0], 0, 4, [epochST, epochED]);
else
    return;
end
if sum(thisEpochCLTS)==0
    try
        thisEpochCLTS = Nlx2MatSpike(['TT' thisTTID '_beh_SS_' thisCLID '_0.ntt'], [1 0 0 0 0], 0, 4, [epochST, epochED]);
    end
end
...
while posRUN <= posN
    if t(posRUN,1) >= thisEpochCLTS(clRUN) && posRUN == 1
        ...
        cont_spk(clRUN,1:size(cont,2)) = cont(posRUN,:);
    ...
    elseif t(posRUN,1) >= thisEpochCLTS(clRUN)
        if t(posRUN,1)-thisEpochCLTS(clRUN) < thisEpochCLTS(clRUN)-t(posRUN-1,1)
            ...
            cont_spk(clRUN,1:size(cont,2)) = cont(posRUN-1,:);
