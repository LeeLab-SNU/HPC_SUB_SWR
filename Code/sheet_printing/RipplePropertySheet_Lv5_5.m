...
ROOT.Fig = [ROOT.Processed '\ripples_mat\ProfilingSheet\R32_' thisRegion];
...
%         FRMap(:,:,find(~id)) = nan;
        ...
        pfn = abs(thisRip.n_afields_L / thisRip.nFields);
        title(['binomial p=' jjnum2str(thisRip.pBinomDev_L_UV,3) ', pSS=' jjnum2str(pf,3) ', pSN=' jjnum2str(pfn,3)])
        ...
        pfn = abs(thisRip.n_afields_R / thisRip.nFields);
           title(['binomial p=' jjnum2str(thisRip.pBinomDev_R_UV,3) ', pSS=' jjnum2str(pf,3) ', pSN=' jjnum2str(pfn,3)])
     set ( gca, 'xdir', 'reverse' )
        ...
        pfn = abs(thisRip.n_afields_C / thisRip.nFields);
          title(['binomial p=' jjnum2str(thisRip.pBinomDev_C_UV,3) ', pSS=' jjnum2str(pf,3) ', pSN=' jjnum2str(pfn,3)])
       set ( gca, 'xdir', 'reverse' )
...
try
         NS_perm = load([ROOT.Rip5 ['\' thisRegion0 '-' thisRip.ID{1} '_max.mat']]);
   
        subplot(9,col,[3 4]*col+5);
        DistPerm(NS_perm.RDI_L.dist.mean,thisRip.mRDI_L_max,NS_perm.RDI_L.p_mean,[-1 1],'mean',0.2)
        ylabel('left scene RDI mean')
end
try
        subplot(9,col,[5 6]*col+5);
        DistPerm(NS_perm.RDI_R.dist.mean,thisRip.mRDI_R_max,NS_perm.RDI_R.p_mean,[-1 1],'mean',0.2)
        ylabel('right scene RDI mean')
end
try
        subplot(9,col,[7 8]*col+5);
        DistPerm(NS_perm.RDI_C.dist.mean,thisRip.mRDI_C_max,NS_perm.RDI_C.p_mean,[-1 1],'mean',0.2)
